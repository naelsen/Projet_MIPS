...
########################################################################
#--- UPDATE~ Pour l'instant on peut placer les pions sur la grille  ---#
#---         chacun son tour et sa fonctionne correctement	    ---#	
#----------------------------------------------------------------------#
#--- LA SUITE~ Il faut maintenant poser les problèmes de limites,   ---#
#---           afficher message d'erreur quand on joue 6 fois	    ---#
#---           sur la même colonne				    ---#
########################################################################
...
FCT_JouerJ2:
	...
	LIRE_J2:
		...
		li $v0, 5
		syscall
		add $s1, $v0, $0# #On sauve pour si ya une erreur
		addi $t1,$v0, -1               # scanf t1 (on fait -1 pour collé a la case de la grille qui est de 0 à 41)
		addi $t3, $0, -1
		bgt $t1, $t3, verif_J2		#
		la $a0, erreur			#
